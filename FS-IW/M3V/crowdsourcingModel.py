...
class CrowdsourcingModel(object):
    def __init__(self, l=3, c=0.25, n=50, maxIter=40, burnIn=10, v=1, alpha=1, TOL=1e-2, seed = None):
        self.l = l
        self.c = c
        self.n = n
    ...
    def loadData(self, filename):
        ...
        self.LabelDomain = np.unique(self.L[self.L != -1])
        self.Ndom = len(self.LabelDomain)
        self.NeibTask = []
        for i in range(self.Ntask):
            tmp = [nt for nt in range(self.Nwork) if self.L[i, nt] > -1]
            self.NeibTask.append(tmp)

        self.NeibWork = []
        for j in range(self.Nwork):
            tmp = [nw for nw in range(self.Ntask) if self.L[nw, j] > -1]
